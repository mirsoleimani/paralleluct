...
//    tbb::task_group g;
    ...
    if (plyOpt.par == SEQUENTIAL) {
        ...
        if (plyOpt.par == PIPEPARFIVE) {
            ...
            ttime = tmr.elapsed();
        } else if (plyOpt.par == PIPEPARNINE) {
            tmr.reset();
            UCTSearchTBBSPSPipe6SB(lstate, 0, 0, tmr);
            ttime = tmr.elapsed();
        } else {
            std::cerr << "No threading library is selected for tree parallelization!\n";
            exit(0);
...
template <class T>
void UCT<T>::UCTSearchTBBSPSPipe6SB(const T& rstate, int sid, int rid, Timer tmr) {
